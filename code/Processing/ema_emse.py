...
for data_path in all_data_files:
    ...
    # 直接刪除指定的 4 個特徵
    features_to_remove = ["ORtg", "DRtg", "+/-", "NRtg"]
    cols_to_smooth_ses = [col for col in ordered_features if col not in features_to_remove]

    print(f"\n→ 已固定刪除特徵：{features_to_remove}")
    print(f"→ 最終保留特徵數：{len(cols_to_smooth_ses)}")

    # 清理 NaN、Inf
    for col in cols_to_smooth_ses:
        df[col] = df[col].replace([np.inf, -np.inf], np.nan).fillna(0).astype(np.float64)
    ...
    train_df_allplayers = df[df["year"].isin(train_years)]
    global_init_df = train_df_allplayers[cols_to_smooth_ses].mean()
    global_init_levels = {
        col: float(global_init_df[col] if not np.isnan(global_init_df[col]) else 0.0)
        for col in cols_to_smooth_ses
    }
