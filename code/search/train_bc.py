...
log_file = os.path.join(log_dir, f'FiN_pairwise_0.0001p2_needAtt_{timestamp}.log')
...
def main():
                ...
                for mlp_hidden_dim in mlp_hidden_dims:
                    for need_att in need_atts:
                        logger.info(f'\n=== Testing combination: Player Dim={player_dim}, '
                                    ...
                                    f'Dropout={dropout_rate},Need Att={need_att}, MLP Hidden={mlp_hidden_dim} (combo_idx={combo_idx}) ===')
                        aucs = []
                        metrics_list = []
                        model_states = []
                        ...
                        for trial_idx in range(num_trials):
                            logger.info(f'Running trial {trial_idx} for Player Dim={player_dim}, '
                                        f'Intermediate Dim={intermediate_dim}, '
                                        f'Dropout={dropout_rate},Need Att={need_att}, MLP Hidden={mlp_hidden_dim}')
                        ...
                        combo_results[(player_dim, intermediate_dim, dropout_rate, mlp_hidden_dim)] = {
                            'avg_auc': avg_auc,
                            'aucs': aucs,
    ...
    for result in best_results:
        logger.info(f"Player Dim: {result['player_dim']}, Intermediate Dim: {result['intermediate_dim']}, "
                   f"Dropout: {result['dropout_rate']},Need Att: {result['need_att']} "
    ...
    logger.info(f"\nBest combination: Player Dim={best_combo[0]}, Intermediate Dim={best_combo[1]}, "
                    f"Dropout={best_combo[2]}, MLP Hidden={best_combo[3]}, "
