...
class FIModel(nn.Module):
    def __init__(self, n_player, player_dim, feature_dim=33, context_dim=1,
                 ...
                 device=torch.device('cpu'), need_att=True):
    ...
    def forward(self, data):
        ...
        # NO-EMA 版本：使用零特徵替代EMA特徵
        feats_A = torch.zeros(batch_size, self.team_size, self.feature_dim, device=self.device)
        feats_B = torch.zeros(batch_size, self.team_size, self.feature_dim, device=self.device)
