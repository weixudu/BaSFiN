...
from BaSFiN_noInter_linear  import NAC               
...
log_file  = os.path.join(log_dir, f"BaSFiN_noInter_nofreeze_linear_2020_{timestamp}.log")
    ...
    *,
    ...
    if stage_idx == 0 and use_pretrain:
        ...
        try:
            ...
            missing, unexpected = model.load_state_dict(state_dict, strict=False)  # ✅ 只有這一行
            logger.info(f"✅ Loaded Stage-0 best model (strict=False): {prev_best_model_path}")
            if missing:      logger.warning(f"Missing keys: {len(missing)}")
            if unexpected:   logger.warning(f"Unexpected keys: {len(unexpected)}")
        except Exception as e:
            logger.warning(f"⚠️ Load Stage-0 failed (shape mismatch): {e}")
            logger.info("Continue training from random initialization")
