...
#            單場比賽拆解（輸出表格用）                  #
# ====================================================== #
def build_model(dataset: Data, device: torch.device) -> NAC:
    """在 main() 外重建模型（設定與 train_and_evaluate 完全一致）"""
    model = NAC(
...
def dump_one_game_effects_by_id(
    *,
    dataset: Data,
    device: torch.device,
    ckpt_path: str,
    ...
    # -------- 在指定 phase 中搜尋該 game_id -------- #
    found = False
    for X, y in dataset.get_batch(1, phase, shuffle=False):
        current_game_id = int(X[0, 0])
        if current_game_id == game_id_target:
            X1 = torch.as_tensor(X, dtype=torch.long, device=device)
            y1 = int(y.flatten()[0])
            found = True
            break

    if not found:
        raise ValueError(f"game_id {game_id_target} not found in phase={phase}")
    
    import pandas as pd

    meta_path = "../data/game_result/nba_games_2009_2024.csv"

    df_meta = pd.read_csv(meta_path)
    df_meta.columns = df_meta.columns.str.strip()

    row_meta = df_meta[df_meta["id"] == game_id_target]

    if len(row_meta) == 0:
        raise ValueError(f"game_id {game_id_target} not found in metadata CSV")

    row_meta = row_meta.iloc[0]

    visitor = row_meta["Visitor"]
    visitor_pts = row_meta["Visitor PTS"]
    home = row_meta["Home"]
    home_pts = row_meta["Home PTS"]
    season = row_meta["Season"]
    date = row_meta["Date"]


    with torch.no_grad():
        # A/B competitive & cooperative
        _, comp_A, comp_B = model.fimodel(X1)
        _, coop_A, coop_B = model.nac_anfm(X1)

        # ind_A/ind_B are REAL team sums from NAC_BBB (sum of BT.mu).
        _, z, ind_A_t, ind_B_t = model.nac_bbb(X1, num_samples=1)
        
        ind_A = float(ind_A_t.item())
        ind_B = float(ind_B_t.item())

        z_scalar = float(z.mean().item())

    out = {
        "phase": phase,
        "game_id": game_id_target,
        "label": y1,
        "ind_A": ind_A,
        "ind_B": ind_B,
        "coop_A": float(coop_A.item()),
        "coop_B": float(coop_B.item()),
        "comp_A": float(comp_A.item()),
        "comp_B": float(comp_B.item()),
        "z": z_scalar,
        "note": "ind_A/ind_B are REAL team sums from NAC_BBB (sum of BT.mu).",
    }

    print("\n========== One-game effects ==========")
    print(f"{season} | {date}")
    print(f"{visitor} ({visitor_pts}) vs {home} ({home_pts})")
    print(f"game_id={game_id_target} | y={y1}")
    print(f"T_A: indiv={ind_A:.4f} | coop={coop_A.item():.4f} | comp={comp_A.item():.4f}")
    print(f"T_B: indiv={ind_B:.4f} | coop={coop_B.item():.4f} | comp={comp_B.item():.4f}")
...
def train_and_evaluate(
    ...
    *,
    ...
    if stage_idx == 0 and use_pretrain:
        ...
        try:
            state_dict = torch.load(prev_best_model_path, map_location=device, weights_only=True)
            model.load_state_dict(state_dict, strict=False)
            logger.info(f"✅ Loaded Stage-0 best model: {prev_best_model_path}")
        except Exception as e:
            logger.warning(f"⚠️ Load failed: {e}, continue training")
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        for step, (X, y) in enumerate(dataset.get_batch(batch_size, "train", shuffle=True), 1):
            ...
            if step % 10 == 0:
                logger.info(
                    f"Stage {stage_idx} | Trial {trial_idx} | Epoch {epoch}/{n_epochs} | "
        ...
        with torch.no_grad():
            for ph in phases:
                ...
                auc, acc, logloss = evaluate_metrics(np.concatenate(preds), np.concatenate(labels))
        ...
        if results[key_phase]["auc"] > best_metric:
            ...
            logger.info(
                f"*** New best {key_phase} AUC {best_metric:.4f} (epoch {epoch}) saved → {save_path}"
            )
        else:
            patience_cnt += 1
...
def main():
    ...
    for f in ["nac_bbb.pth", "fimodel.pth", "anfm.pth"]:
        if not os.path.exists(os.path.join(model_save_dir, f)):
            logger.error(f"Pre-trained model missing: {f}")
            return
    ...
    for trial_idx in range(NUM_TRIALS):
        ...
        logger.info(
            f"[Trial {trial_idx}] n_individual={dataset.n_individual} | "
        ...
        if metric is not None:
            ...
            best_model_path = os.path.join(
                save_dir,
                f"nac_prob{prob_dim}_drop{dropout}_lr{learning_rate}_SGD_stage0_trial{trial_idx}.pth"
            )
        else:
        ...
        logger.info(
            f"=== Trial {trial_idx} completed : "
            f"Best Val AUC {best_val_auc:.4f} | Best Test AUC {best_test_auc:.4f} ==="
        )

        # ====== 直接在這裡輸出一場比賽的表格數值 ======
        ckpt_stage1 = os.path.join(
            save_dir,
            f"nac_prob{prob_dim}_drop{dropout}_lr{learning_rate}_SGD_stage1_trial{trial_idx}.pth"
        )
        try:
            dump_one_game_effects_by_id(
                dataset=dataset,
                device=device,
                ckpt_path=ckpt_stage1,
                phase="test",
                game_id_target=19161,   # ← 直接指定
)
        except Exception as e:
            logger.warning(f"dump_one_game_effects failed: {e}")
