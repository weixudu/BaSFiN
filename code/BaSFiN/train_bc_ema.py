...
log_file = os.path.join(log_dir, f'FiN_bc_ReLU10_{timestamp}.log')
...
def train_and_evaluate(player_dim, intermediate_dim, dropout_rate, mlp_hidden_dim, dataset, 
                      ...
                      ema_tensor_path, game_id_mapping, combo_idx, trial_idx, need_att, phase='step1'):
    seed = SEED + trial_idx
    random.seed(seed)
    np.random.seed(seed)
    torch.manual_seed(seed)
...
def main():
        ...
        for intermediate_dim in intermediate_dims:
            for dropout_rate in dropout_rates:
                for mlp_hidden_dim in mlp_hidden_dims:
                    for need_att in need_atts:  # Include need_att in the loop
    ...
    for trial_idx in range(num_trials):
        ...
        test_auc, metrics, model_state, test_preds, test_labels = train_and_evaluate(
            ...
            dataset, game_ids_train, game_ids_valid, game_ids_test,
            ema_tensor_path, game_id_mapping, combo_idx=0, trial_idx=trial_idx, 
            need_att=best_combo[4], phase='step2'  # Pass the best need_att value
        )
    ...
    # Load best model with correct need_att
    best_model = FIModel(
        n_player=dataset.n_individual,
        ...
        ema_tensor_path=ema_tensor_path,
        game_id_mapping_path=game_id_mapping_path,
        need_att=best_combo[4]  # Use the best need_att value
    ).to(device)
