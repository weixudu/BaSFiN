...
def main():
    ...
    for end_year in ["2020", "2021", "2022", "2023", "2024"]:
        ...
        year_log_dir = os.path.join('logs/NAC', f'{start_year}_{end_year}')
        os.makedirs(year_log_dir, exist_ok=True)

        # 獨立 log 檔
        time_tag = datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
        log_file = os.path.join(year_log_dir, f'NAC_training_{time_tag}.log')
        ...
        for trial_idx in range(num_trials):
            ...
            val_auc, metrics, skill_mu = train_and_evaluate(
                dataset, n_epochs, batch_size, learning_rate, device, early_stop_patience,
                trial_idx, phase='step1'
            )
            ...
            trial_best_paths_step1.append(
                os.path.join(year_log_dir, f'best_model_trial{trial_idx}_step1_{time_tag}.pth')
            )
        ...
        for trial_idx in range(num_trials):
            ...
            trial_best_paths_step2.append(
                os.path.join(year_log_dir, f'best_model_trial{trial_idx}_step2_{time_tag}.pth')
            )
