...
log_file = os.path.join(log_dir, f'BT_training_{current_time}.log')
logging.basicConfig(
    level=logging.INFO,
...
def main():
    ...
    for end_year in ["2020", "2021", "2022", "2023", "2024"]:
        ...
        # 路徑動態指定
        year_path = f'../data/final_data/data_{start_year}_{end_year}.csv'
        year_output_dir = os.path.join('../output/BT', f'{start_year}_{end_year}')
        year_model_dir = os.path.join(year_output_dir, 'models')
        year_log_dir = os.path.join('logs/BT', f'{start_year}_{end_year}')
        ...
        log_file = os.path.join(year_log_dir, f'BT_training_{time_tag}.log')
        file_handler = logging.FileHandler(log_file)
        file_handler.setFormatter(logging.Formatter('%(asctime)s - %(levelname)s - %(message)s', datefmt='%H:%M:%S'))
        logger.addHandler(file_handler)
        
        logger.info("\n" + "="*80)
        logger.info(f"開始處理年份視窗：{start_year} ~ {end_year}")
        logger.info("="*80)

        if not os.path.exists(year_path):
            logger.error(f"❌ Data file not found: {year_path}")
            continue
        
        # 載入資料
        dataset = Data(year_path, team_size=team_size, seed=SEED)
        ...
        for trial_idx in range(num_trials):
            ...
            val_auc, metrics, skill_mu, _, _ = train_and_evaluate(
                dataset, n_epochs, batch_size, learning_rate, device,
                early_stop_patience, trial_idx, phase='step1'
            )
        ...
        for trial_idx in range(num_trials):
            ...
            test_auc, metrics, skill_mu, test_preds, test_labels = train_and_evaluate(
                dataset, n_epochs, batch_size, learning_rate, device,
                early_stop_patience, trial_idx, phase='step2'
            )
        ...
        df_avg.to_csv(os.path.join(year_output_dir, f'test_avg_preds_labels_step2_{time_tag}.csv'), index=False)
        logger.info(f"Saved average test preds and labels to CSV.")
        ...
        # 移除這一輪的 file handler，避免多次輸出到錯誤檔
