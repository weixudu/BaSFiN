...
def elbo_loss(model, X, y, kl_w, ns):
    ...
    prob, _ = model(X, num_samples=ns)
    ll = -nn.BCELoss(reduction='sum')(prob, y_rep) / ns
    return -ll + kl_w * model.kl_divergence()

def evaluate(pred, label):
    pred = pred.reshape(-1); label = label.reshape(-1)
...
def train_and_evaluate(kl_w, lr, dataset, idx, trial):
    ...
    for ep in range(1, n_epochs+1):
        ...
        # --------- 驗證 / 測試 ----------
        model.eval(); metrics_dict = {}
        with torch.no_grad():
            for ph in phases:
                ps, labs = [], []
                for X, y in dataset.get_batch(batch_size, ph):
                    X = torch.tensor(X, dtype=torch.long, device=device)
                    p,_ = model(X, num_samples=num_samples)
                    ps.append(p.mean(0).cpu().numpy()); labs.append(y.flatten())
                auc, acc, ll = evaluate(np.concatenate(ps), np.concatenate(labs))
                metrics_dict[ph] = dict(auc=auc, acc=acc, logloss=ll)
                logger.info(f"    {ph.capitalize():5s} AUC {auc:.4f} Acc {acc:.4f} "
                            f"LogLoss {ll:.4f}")
...
# ================= Main ========================
def main():
    #大LR 預測較準(推測可以快速學習新球員)，但就要搭配小KL，才能收斂sigma
    ...
    if best_cfg:
        ...
        logger.info("no valid set → use test AUC ")
