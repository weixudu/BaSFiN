...
player_dims = [16, 32, 64]
hidden_dims = [16, 32, 64]
need_atts = [True, False]
weight_decay = 0.005
...
log_file = os.path.join(log_dir, f'cofim_0.0001p2_{timestamp}.log')
...
def main():
            ...
            for need_att in need_atts:
                for dropout_rate in dropout_rates:
                    for mlp_hidden_dim in mlp_hidden_dims:
                        logger.info(f'\n=== Testing combination: Player Dim={player_dim}, Hidden Dim={hidden_dim}, '
                                   f'Need Att={need_att}, Dropout={dropout_rate}, MLP Hidden={mlp_hidden_dim}, '
                                   f'Weight Decay={weight_decay} (combo_idx={combo_idx}) ===')
                        aucs = []
                        metrics_list = []
                        model_states = []
    ...
    for result in best_results:
        logger.info(f"Player Dim: {result['player_dim']}, Hidden Dim: {result['hidden_dim']}, "
                   f"Need Att: {result['need_att']}, Dropout: {result['dropout_rate']}, "
    ...
    logger.info(f"\nBest combination: Player Dim={best_combo[0]}, Hidden Dim={best_combo[1]}, "
               f"Need Att={best_combo[2]}, Dropout={best_combo[3]}, MLP Hidden={best_combo[4]}, "
    ...
    # Test forward pass to get A_coop and B_coop
    with torch.no_grad():
        sample_data = torch.randint(0, dataset.n_individual, (1, team_size * 2 + 1)).to(device)  # Example data
