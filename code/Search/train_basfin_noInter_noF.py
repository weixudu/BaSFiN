...
learning_rate = 0.00005   # 固定學習率
freeze_modules = False    # 從頭訓練，不需凍結
use_pretrain = False      # *** 重要：不使用預訓練權重 ***
num_trials = 5
...
model_save_dir     = "model/pretrain_BaSFiN_model"  # 仍用於儲存新的權重
save_dir           = "model/NAC_plus"
log_dir            = "logs/NAC+"

# ---------------------------------------------------------------------------
# Logging 設定（保留原格式與路徑，不會刪除舊檔）
# ---------------------------------------------------------------------------
os.makedirs(log_dir, exist_ok=True)
os.makedirs(save_dir, exist_ok=True)
log_file = os.path.join(log_dir, f'BaSFiN_noInter_noFreeze_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log')
logging.basicConfig(
    level=logging.INFO,
...
# ---------------------------------------------------------------------------
# 訓練與驗證主函式（新增 force_no_freeze 預設值，並將 use_pretrain 改為 False）
# ---------------------------------------------------------------------------
...
def train_and_evaluate(dataset, n_epochs, batch_size, learning_rate, num_samples, device,
                      ...
                      dropout, combo_idx, trial_idx, bc_need, force_no_freeze=False):
    # 固定亂數種子，確保可重現性
    seed = SEED + trial_idx
    random.seed(seed)
    np.random.seed(seed)
...
def main():
    ...
    for prob_dim in range(8, 129):
        dropout = 0.2  # 固定 dropout，此處可自行調整
        ...
        for trial_idx in range(num_trials):
            ...
            val_auc, results, skill_mu, skill_sigma, info = train_and_evaluate(
                ...
                early_stop_patience, use_pretrain=False, freeze_modules=False,
        ...
        if aucs:
            ...
            best_results_summary.append({
                ...
                'avg_auc': avg_auc,
                'metrics': metrics_list,
                'best_trial_idx': int(np.argmax(aucs)) if aucs else 0
            })
        combo_idx += 1
    ...
    best_result = next(r for r in best_results_summary if (r['prob_dim'], r['dropout'], r['learning_rate']) == best_combo)
