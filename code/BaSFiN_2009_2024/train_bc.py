...
def accumulate_pair_stats(stats_all, stats_focus,
    ...
    for s, a_idx, b_idx in zip(pair_scores.tolist(), ids_A_pair, ids_B_pair):
        ...
        if focus_idx is not None and (a_idx == focus_idx or b_idx == focus_idx):
        ...
        with torch.no_grad():
            for X, _ in loader:
                ...
                for b in range(X.shape[0]):
                    accumulate_pair_stats(stats_AD_all_tv, stats_AD_focus_tv,
                                        pair1[b].cpu(), team_A[b], team_B[b],
                                        focus_idx, idx_to_pid)
                    accumulate_pair_stats(stats_DA_all_tv, stats_DA_focus_tv,
